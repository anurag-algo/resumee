...
export default function FileUpload({ onAnalysisComplete, onError }) {
  ...
  const hasEnoughCredits = balance === null || balance >= ANALYSIS_COST;
  const isLowBalance = balance !== null && balance < ANALYSIS_COST;
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      // Refresh wallet so the new balance (after deduction) is reflected
      refreshWallet();
  ...
  return (
    <>
      ...
      >
        ...
        {/* Credit cost / low balance banner */}
        <div
        ...
        >
          <div className="flex items-center gap-2">
            <Zap
              aria-hidden="true"
              size={16}
              className={isLowBalance ? "text-amber-300" : "text-cyan-300"}
            />
            <span className="font-medium">
              Analysis Cost:{" "}
              <span className="font-bold text-slate-100">
                {ANALYSIS_COST} Credits
              </span>
            </span>
          </div>
          {balance !== null && (
            <span
              className={`text-xs font-semibold ${
                isLowBalance ? "text-amber-300" : "text-slate-400"
              }`}
            >
        ...
        {/* Low balance warning */}
        {isLowBalance && (
          <div className="mb-5 flex items-start justify-between gap-3 rounded-xl border border-amber-400/30 bg-amber-400/10 px-4 py-3 text-sm text-amber-200">
            <div className="flex items-start gap-2">
              ...
              />
              <span>
                You don&apos;t have enough credits to analyze. You need{" "}
                <strong>{ANALYSIS_COST}</strong> credits but only have{" "}
                <strong>{balance}</strong>.
              </span>
            </div>
            <button
              type="button"
              id="fileupload-buy-credits-btn"
              onClick={() => setBuyModalOpen(true)}
              className="shrink-0 inline-flex items-center gap-1.5 rounded-lg bg-amber-400 px-3 py-1.5 text-xs font-bold text-slate-950 transition hover:bg-amber-300"
            >
        ...
        {isLowBalance ? (
          <button
            type="button"
            id="fileupload-buy-credits-submit-btn"
            onClick={() => setBuyModalOpen(true)}
            className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-amber-400 px-4 py-3 text-sm font-bold text-slate-950 transition hover:bg-amber-300"
          >
